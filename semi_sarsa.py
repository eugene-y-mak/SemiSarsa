...
class SemiSarsa:
    def __init__(self, env, n_step=8, alpha=.3/8, epsilon=0.1, gamma=0.9):
        self.env = env
        self.n_step = n_step
    ...
    def run(self):
        num_episodes = 0
        while num_episodes < 10000:  # per episode
            episode_reward = 0
            ...
            T = np.inf
            t = 0
            actions = np.empty(self.store_size)
            rewards = np.empty(self.store_size)
            states = np.empty([self.store_size, state_dimension])
